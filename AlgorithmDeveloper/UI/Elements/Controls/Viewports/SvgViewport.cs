...
namespace AlgorithmDeveloper.UI.Elements.Controls.Viewports
{
    public partial class SvgViewport : UserControl
    {
        ...
        private void AppendLabel(StringBuilder sb, IFigure f, float x, float y)
        {
            ...
            sb.Append($"<text x='{x}' y='{y}' text-anchor='middle' dominant-baseline='middle' fill='{VisualizationSettings.FigureTextColorHex}' font-family='Segoe Elements, Arial' font-size='{VisualizationSettings.FontSize}'>" + Escape(id) + "</text>");
        }
        ...
        private void AppendSmallLabel(StringBuilder sb, IFigure f, float x, float y)
        {
            string id = GetFigureId(f);
            if (string.IsNullOrEmpty(id)) return;
            sb.Append($"<text x='{x}' y='{y}' fill='{VisualizationSettings.FigureTextColorHex}' font-family='Segoe Elements, Arial' font-size='{Math.Max(10, VisualizationSettings.FontSize - 6)}'>" + Escape(id) + "</text>");
        }
