...
namespace AlgorithmDeveloper.AAModel.TransitionSystem.MAS
{
    ...
    public sealed class MatrixAlgorithmSchema
    {
        ...
        public static MatrixAlgorithmSchema FromFormulas(in List<TransitionFormula> formulas, OutputView view = OutputView.LogicalExpression)
        {
            ...
            // 1) Определяем порядок строк: Yн первым, затем по возрастанию ID
            var ordered = formulas
                .OrderBy(f => f.YStart is StartVertex ? 0 : 1)
                .ThenBy(f => f.YStart.ID, StringComparer.Ordinal)
                .ToList();

            // 2) Заголовки столбцов: все YStart.ID в указанном порядке + Yк (если есть среди целей)
            var headers = ordered.Select(f => f.YStart.ID ?? string.Empty).ToList();
            var endId = ordered
                .SelectMany(f => f.Conditions)
                .Where(c => c.YTo is EndVertex)
                .Select(c => c.YTo.ID ?? string.Empty)
                .Distinct()
                .FirstOrDefault();
            if (!string.IsNullOrEmpty(endId) && !headers.Contains(endId))
                headers.Add(endId); // колонка Yк в конце
            ...
            foreach (var f in ordered)
            {
                ...
                // Группируем условия по целевой вершине, сортируем группы согласно headers
        ...
        private static string RenderCell(List<TransitionCondition>? conditions, OutputView view)
        {
            ...
            {
                // В ячейке отображаем только логическую часть (без YTo), объединяя термы через "˅".
